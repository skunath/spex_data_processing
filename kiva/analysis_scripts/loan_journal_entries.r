...
query2 <- "
          select kiva_id, counter, @rank_counter := @rank_counter + 1 as rank from (
          SELECT a.kiva_id, count(*) as counter FROM loans a
...
t <- ggplot(loan_journals, aes(x = rank, y= counter))

...
query1 <- "select sector, datediffer, avg(funded_amount) as avg_funded, count(*) as counter from (
            select sector, country, funded_amount, posted_date, funded_date,
            datediff(funded_date, posted_date) as datediffer 
            from loans a
            where datediff(funded_date, posted_date) >= 0
            ) z
            group by sector, datediffer"

res<-dbSendQuery(con, query1)
loan_journals <- fetch(res, n = -1)
#t <- qplot(datediffer, data=loan_journals, geom="histogram", binwidth=1)
t <- ggplot(loan_journals, aes(x = datediffer, y=avg_funded, group=sector, color=sector), binwidth=1)

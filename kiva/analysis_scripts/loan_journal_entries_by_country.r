...
query2 <- "set @country := null;"

query3 <- "select * from (
            select kiva_id, counter, @rank_counter := if(@country = country, @rank_counter + 1, @rank_counter := 1) as rank, 
            @country := country as country from (
            SELECT a.kiva_id, country, count(*) as counter FROM loans a
            left outer join journal_comments b on b.kiva_loan_id = a.kiva_id
            where b.kiva_loan_id is not null
            group by a.kiva_id
            order by country, counter desc
            ) f ) g
            where rank <= 10000"
...
t <- ggplot(loan_journals, aes(x = rank, y= counter, group=country, colour=country))
